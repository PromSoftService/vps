...
validate_mux_settings() {
  validate_yes_no XRAY_MUX_ENABLED "${XRAY_MUX_ENABLED:-no}"
...
generate_xray_config() {
  ...
  validate_yes_no CLIENT_UDP_ENABLED "$CLIENT_UDP_ENABLED"
  validate_yes_no CLIENT_SNIFF_QUIC "$CLIENT_SNIFF_QUIC"
  validate_mux_settings
...
XRAY_CLIENT_ALPN=${XRAY_CLIENT_ALPN}
CLIENT_UDP_ENABLED=${CLIENT_UDP_ENABLED}
...
EOF2
  ...
  client_args=(
    ...
    --doh "$DOH_URL"
    --remark "$DOMAIN ws h3 fallback"
    --fingerprint "$XRAY_FINGERPRINT_VALUE"
    --alpn "$XRAY_CLIENT_ALPN"
    --client-udp-enabled "$CLIENT_UDP_ENABLED"
    --client-sniff-quic "$CLIENT_SNIFF_QUIC"
...
main() {
  ...
  log "Generating xray WS config and client artifacts"
  generate_xray_config
  ...
  if cert_present; then
    ...
    echo "Web fallback: https://${DOMAIN}/"
    echo "WS endpoint: wss://${DOMAIN}${XRAY_PATH_NORMALIZED}"
